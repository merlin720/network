...
public final class Phoenix {
    ...
    public static void init(Context context) {
        MLog.init(true, "MLog");
        Fresco.initialize(context, FImageLoaderConfig.getImagePipelineConfig(context));
    }
    ...
    public static class Builder {
        ...
        public void download() {
            ...
            com.facebook.fresco.helper.ImageLoader.downloadImage(mContext, mUrl, mDownloadResult);
        }
        ...
        public void load() {
            ...
            // 目前只对从网络加载图片，提供支持
            com.facebook.fresco.helper.ImageLoader.loadImage(mContext, mUrl,
                    mWidth,
                    mHeight,
                    new IResult<Bitmap>() {
        ...
        public void load(int resId) {
            ...
            if(!mNeedBlur) {
                if (mWidth > 0 && mHeight > 0) {
                    com.facebook.fresco.helper.ImageLoader.loadDrawable(mSimpleDraweeView, resId, mWidth, mHeight);
                } else if (handlerAspectRatio()) {
                    com.facebook.fresco.helper.ImageLoader.loadDrawable(mSimpleDraweeView, resId);
                }
            } else {
                if (mWidth > 0 && mHeight > 0) {
                    com.facebook.fresco.helper.ImageLoader.loadDrawableBlur(mSimpleDraweeView, resId, mWidth, mHeight);
                } else if (handlerAspectRatio()) {
                    com.facebook.fresco.helper.ImageLoader.loadDrawableBlur(mSimpleDraweeView, resId);
                }
            }
        }
        ...
        private void loadNormal(String url) {
            ...
            com.facebook.fresco.helper.ImageLoader.loadImage(mSimpleDraweeView, uri, mWidth, mHeight, mPostprocessor,
        ...
        private void loadBlur(String url) {
            ...
            if(mWidth > 0 && mHeight > 0) {
                if (UriUtil.isNetworkUri(uri)) {
                    com.facebook.fresco.helper.ImageLoader.loadImageBlur(mSimpleDraweeView, url, mWidth, mHeight);
                } else {
                    com.facebook.fresco.helper.ImageLoader.loadFileBlur(mSimpleDraweeView, url, mWidth, mHeight);
                }
            } else if(handlerAspectRatio()) {
                if (UriUtil.isNetworkUri(uri)) {
                    com.facebook.fresco.helper.ImageLoader.loadImageBlur(mSimpleDraweeView, url);
                } else {
